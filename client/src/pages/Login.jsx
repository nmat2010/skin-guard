                ...
                if (data.success) 
                    {
                        ...
                        getUserData()
                        navigate('/')
                        console.log("Login in with data: ", data)
                    }
                else {
                    toast.error(data.message)
                }
    ...
    <div className='login'>
        <div className='login-upper-container'>
            <div className='login-container'>
                ...
                <form onSubmit={onSubmitHandler}>
                    {state === 'Sign Up' && (
                        <div className='login-content'>
                            ...
                            id="name" type="text" className='input-field' required></input>
                        </div>
                    )}
                    <div className='login-content'>
                        ...
                        id="email" type="email" className='input-field' required></input>
                    </div>
                    <div className='login-content'>
                        <label for="password" className='input-label'>Password</label>
                        <br></br>
                        <input onChange={e => setPassword(e.target.value)} value={password}
                        id="password" type="text" className='input-field' required></input>
                    </div>
                    {state === 'Log In' && (
                        <Link to='/reset-password'><p className='forget-pass'>Forget Password?</p></Link>
                    )}
                    <div className='login-button'>
                        <button id="login-btn" type="submit">{state}</button>
                    </div>
                </form>
                ...
                {state === 'Sign Up' ? (
                    <p>Already have an account? <span onClick={() => setState('Log In')} className='login-link'>Log in</span></p>
                ) : (
                    <p>Don't have an account? <span onClick={() => setState('Sign Up')} className='signup-link'>Sign up</span></p>
                )}
            </div>
        </div>
    </div>
